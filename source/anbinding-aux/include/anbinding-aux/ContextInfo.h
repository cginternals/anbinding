...
*/
class ANBINDING_AUX_API ContextInfo
{
public:
    /**
    *  @brief
    ...
    *    The list of available extensions known by glbinding
    */
    static std::set<an::AnExtension> extensions(std::set<std::string> & unknown);
    ...
    */
    static bool supported(const std::set<an::AnExtension> & extensions);
    ...
    *    'true' if all given extensions are supported by the current context, else 'false'
    */
    static bool supported(const std::set<an::AnExtension> & extensions , std::set<an::AnExtension> & unsupported);
